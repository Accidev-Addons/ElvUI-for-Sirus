...
local texturePath = [[Interface\AddOns\ElvUI_Libraries\Core\LibCustomGlow-1.0\]]
...
local FramePoolResetter = function(framePool, frame)
	...
	if parent[frame.name] then parent[frame.name] = nil end
...
local function addFrameAndTex(r, color, name, key, N, xOffset, yOffset, texture, texCoord, desaturated, frameLevel)
	...
	f:Show()
	if not f.textures then f.textures = {} end
	for i = 1, N do
...
function lib.PixelGlow_Stop(r, key)
	if not r then return end
	key = key or ""
	if not r["_PixelGlow"..key] then return false
	else GlowFramePool:Release(r["_PixelGlow"..key]) end
end
...
function lib.AutoCastGlow_Start(r, color, N, frequency, scale, xOffset, yOffset, key, frameLevel)
	...
	for k, size in pairs(acSizes) do
		for i = 1, N do f.textures[i + N * (k - 1)]:SetSize(size * scale, size * scale) end
	end
...
function lib.AutoCastGlow_Stop(r, key)
	if not r then return end
	key = key or ""
	if not r["_AutoCastGlow"..key] then return false
	else GlowFramePool:Release(r["_AutoCastGlow"..key]) end
end
...
local function AlphaAnimation_OnUpdate(self, elapsed)
	...
	if progress ~= 0 then
		...
		if self.frameAlpha then
			...
			if progress == 1 then TidyAlphaAnimation(self) end
		end
	end
end
local function AlphaAnimation_OnStop(self)
	if self.frameAlpha then TidyAlphaAnimation(self) end
	self.played = nil
end
local function CreateAlphaAnim(group, target, order, duration, change, delay, onPlay, onFinished, appear)
	...
	if target then alpha.target = alpha:GetRegionParent()[target] end
	if order then alpha:SetOrder(order) end
	alpha:SetDuration(duration)
	alpha.change = appear and change or -change
	if delay then alpha:SetStartDelay(delay) end
	if onPlay then alpha:SetScript("OnPlay", onPlay) end
...
local function ScaleAnimation_OnUpdate(self, elapsed)
	...
	if progress ~= 0 then
		if not self.played then
			if InitScaleAnimation(self) then self.played = 1 end
		end
		if self.frameWidth then
			...
			if progress == 1 then TidyScaleAnimation(self) end
		end
	end
end
local function ScaleAnimation_OnStop(self)
	if self.frameWidth then TidyScaleAnimation(self) end
	self.played = nil
end
local function CreateScaleAnim(group, target, order, duration, x, y, delay, smoothing, onPlay)
	local scale = group:CreateAnimation()
	if target then scale.target = scale:GetRegionParent()[target] end
	scale:SetOrder(order)
	scale:SetDuration(duration)
	scale.scaleX, scale.scaleY = x, y
	if delay then scale:SetStartDelay(delay) end
	if smoothing then scale:SetSmoothing(smoothing) end
	if onPlay then scale:SetScript("OnPlay", onPlay) end
...
local function AnimateTexCoords(texture, textureWidth, textureHeight, frameWidth, frameHeight, numFrames, elapsed, throttle)
	...
	if not texture.throttle or texture.throttle > throttle then
		...
		while frame + framesToAdvance > numFrames do frame = frame - numFrames end
...
local function ButtonGlowResetter(framePool,frame)
	local parent = frame:GetParent()
	if parent._ButtonGlow then parent._ButtonGlow = nil end
	frame:Hide()
	frame:ClearAllPoints()
end
...
local function IsAnimPlaying(self) return self.isPlaying end
...
local function updateAlphaAnim(f,alpha)
	for _,anim in pairs(f.animIn.appear) do anim.change = alpha end
	for _,anim in pairs(f.animIn.fade) do anim.change = -alpha end
	for _,anim in pairs(f.animOut.appear) do anim.change = alpha end
	for _,anim in pairs(f.animOut.fade) do anim.change = -alpha end
end
...
local function noZero(num)
	if num == 0 then return 0.001 else return num end
end
...
function lib.ButtonGlow_Start(r, color, frequency, frameLevel)
	...
	if r._ButtonGlow then
		...
		if new then configureButtonGlow(f,color and color[4] or 1)
		else updateAlphaAnim(f,color and color[4] or 1) end
		r._ButtonGlow = f
		local width, height = r:GetSize()
		f:SetParent(r)
		f:SetFrameLevel(r:GetFrameLevel() + frameLevel)
		...
		if not color then
			f.color = false
			for texture in pairs(ButtonGlowTextures) do f[texture]:SetVertexColor(1, 1, 1) end
		else
			f.color = color
			for texture in pairs(ButtonGlowTextures) do f[texture]:SetVertexColor(color[1], color[2], color[3]) end
		end
		f.throttle = throttle
...
local StartFlipbook, FlipbookAnimation_OnUpdate
FlipbookAnimation_OnUpdate = function(self, elapsed)
	...
	if data.animElapsed then
		...
		if data.animElapsed >= 0.7 then
			if self:IsShown() then StartFlipbook(self, self.ProcLoop, 6, 5, 30, ((data.animOptions and (30 / data.animOptions)) or 30), nil, nil, "Loop") end
	...
	if data.elapsedTime >= frameDuration then
		...
		if data.currentFrame > data.totalFrames then data.currentFrame = 1 end
		SetTile(data.texture, data.currentFrame, data.rows, data.columns, 1, 1, data.key)
	end
end
local function StopFlipbook(f)
	f:SetScript("OnUpdate", nil)
	if f.flipbookData and f.flipbookData.texture then f.flipbookData.texture:Hide() end
	f.flipbookData = nil
end
...
local function SetupProcGlow(f, options)
	...
	f:SetScript("OnHide", function(self) StopFlipbook(self) end)
...
function lib.ProcGlow_Stop(r, key)
	...
	if f then ProcGlowPool:Release(f) end
end
