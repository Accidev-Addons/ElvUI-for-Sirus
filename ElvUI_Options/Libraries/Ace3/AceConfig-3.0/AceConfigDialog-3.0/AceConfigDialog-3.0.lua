...
-- Lua APIs
local tinsert, tsort, tremove, tconcat, wipe = table.insert, table.sort, table.remove, table.concat, table.wipe
local strmatch, format, loadstring = string.match, string.format, loadstring
local error = error
...
function AceConfigDialog:SelectGroup(appName, ...)
	...
	for n = 1, select("#",...) do
		...
		if group.childGroups == "tab" or group.childGroups == "select" or group.childGroups == "ElvUI_HiddenTree" then
			...
			if v.type == "group" then
				...
				elseif v.type == "select" then
					...
					if v.style == "radio" then
						...
						control:SetLabel(name)
						control:SetList(values, sorting, nil, sortByValue)
						local value = GetOptionsMemberValue("get",v, options, path, appName)
