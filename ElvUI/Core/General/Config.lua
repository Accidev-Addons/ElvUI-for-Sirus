...
function E:CreateMoverPopup()
	...
	nudgeFrame:HookScript('OnShow', function(self) self:EnableKeyboard(true) end)
	nudgeFrame:HookScript('OnHide', function(self) self:EnableKeyboard(false) end)
...
function E:ToggleOptions(msg)
	...
	if not IsAddOnLoaded('ElvUI_Options') then
		local _, _, _, _, _, reason = GetAddOnInfo('ElvUI_Options')
