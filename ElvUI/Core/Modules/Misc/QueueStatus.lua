...
function M:HideQueueStatus()
	local display = M.QueueStatusDisplay
	display.text:SetText('')
	display:SetScript('OnUpdate', nil)
end

function M:RestoreQueueStatus()
	local display = M.QueueStatusDisplay
	if display.title and display.queuedTime then
		display.updateThrottle = 0
		display:SetScript('OnUpdate', M.QueueStatusOnUpdate)
	end
end
...
function M:CreateQueueStatusText()
	...
	queueButton:HookScript('OnHide', M.ClearQueueStatus)
...
local repinning
local function QueueStatusRepinButton(button)
	if repinning or not M.QueueStatus then return end
	repinning = true

	if button:GetParent() ~= M.QueueStatus then
		button:SetParent(M.QueueStatus)
	end

	local _, relTo = button:GetPoint()
	if relTo ~= M.QueueStatus then
...
local function QueueStatusRepinFrame(frame)
	if repinning or not M.QueueStatus then return end
	repinning = true

	frame:ClearAllPoints()
	frame:SetPoint('TOP', M.QueueStatus, 'BOTTOM', 0, -4)
...
function M:HandleQueueStatus(creation)
	local queueButton = M:GetQueueStatusButton()
	if not queueButton or not M.QueueStatus then return end

	if creation then
		queueButton:SetParent(M.QueueStatus)
		queueButton:SetPoint('CENTER')

		hooksecurefunc(queueButton, 'SetParent', QueueStatusRepinButton)
		hooksecurefunc(queueButton, 'SetPoint', QueueStatusRepinButton)

		local statusFrame = _G.QueueStatusFrame
		if statusFrame then
			hooksecurefunc(statusFrame, 'SetPoint', QueueStatusRepinFrame)
			statusFrame:HookScript('OnShow', QueueStatusRepinFrame)
		end
	end
	...
	if queueDisplay then
		...
		queueDisplay.text:Point(db.position, queueButton, db.position, db.xOffset, db.yOffset)
		queueDisplay.text:FontTemplate(LSM:Fetch('font', db.font), db.fontSize, db.fontOutline)

		if not db.enable then
			M:HideQueueStatus()
		elseif queueDisplay.title then
			M:RestoreQueueStatus()
		end
	end
end
...
function M:LoadQueueStatus()
	if not E.private.general.queueStatus or not E.private.general.minimap.enable then return end
	...
	if M:GetQueueStatusButton() then
